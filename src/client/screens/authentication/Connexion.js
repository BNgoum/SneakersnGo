...
class Connexion extends Component {
    ...
    checkInputNotBlank = () => {
        if ( this.state.email === "" && this.state.password === "" ) {
            ...
            this.setState({ isEmailBlank: true })
        }
        else if ( this.state.password === "" ) {
            this.setState({ isPasswordBlank: true })
        }
        else {
            return new Promise((resolve, reject) => {
                resolve(requestLogin(this.state.email, this.state.password))
            })
            .then((action) => {
    ...
    render() {
        return (
            <View style={ styles.wrapperInscription }>
                ...
                { this.props.state.auth_message_error !== "" ? <Text style={styles.textError}>{this.props.state.auth_message_error}</Text> : null }
